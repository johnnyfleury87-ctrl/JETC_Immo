...
export async function getProfile() {
  ...
  console.log('[getProfile] Récupération profile pour user:', session.user.id);
  ...
  if (profileError) {
    console.error('[getProfile] ❌ Erreur récupération profile:', profileError);
    ...
    console.error('[getProfile] Hint:', profileError.hint);
    
    // Si PGRST116 = No rows found
    if (profileError.code === 'PGRST116') {
      console.error('[getProfile] ⚠️ Profile existe mais RLS bloque l\'accès');
      console.error('[getProfile] 💡 Vérifier : SELECT * FROM profiles WHERE id = auth.uid();');
  ...
  if (!profile) {
    ...
    console.error('[getProfile] 💡 Le profile existe dans la table mais RLS retourne NULL');
    throw new Error('Profil introuvable (RLS?)');
  }

  // 3. Vérifier que le role existe (donnée critique)
  if (!profile.role) {
    console.error('[getProfile] Profile sans role, userId:', profile.id);
    throw new Error('Profil incomplet (pas de role)');
  }

  // ✅ LOG SÉCURISÉ : Pas d'email (RGPD)
