...
export async function markAsRead(req, res) {
  try {
    ...
    if (updateError) {
      ...
      return res
        .status(500)
        .json({ error: "Erreur lors du marquage de la notification" });
    }
...
export async function markAllAsRead(req, res) {
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Erreur lors du marquage des notifications" });
    }
  ...
  try {
    ...
    if (!user_id || !type_notification || !titre || !message) {
      return res.status(400).json({
        error:
          "Les champs user_id, type_notification, titre et message sont requis",
      });
    }
    ...
    if (profileError || !profile || profile.role !== "admin_jtec") {
      return res.status(403).json({
        error:
          "Seuls les administrateurs peuvent créer des notifications manuellement",
      });
    }
