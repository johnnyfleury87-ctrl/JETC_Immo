  ...
  try {
    ...
    if (profile.role === 'regie' || profile.role === 'locataire') {
      ...
      if (regie_id) {
        ...
        return res.status(400).json({ error: 'Admin doit spécifier regie_id ou entreprise_id' });
      }
    }
  ...
  try {
    ...
    if (profile.role === 'regie') {
      ...
      if (regie_id) {
        ...
        return res.status(400).json({ error: 'Spécifier regie_id ou entreprise_id' });
      }
    }

    // Ajouter les champs modifiables
    if (delai_reponse_max_heures !== undefined) updates.delai_reponse_max_heures = delai_reponse_max_heures;
    if (priorites_actives !== undefined) updates.priorites_actives = priorites_actives;
    if (categories_personnalisees !== undefined) updates.categories_personnalisees = categories_personnalisees;
    if (auto_assignation_tickets !== undefined) updates.auto_assignation_tickets = auto_assignation_tickets;
    if (duree_intervention_defaut_minutes !== undefined) updates.duree_intervention_defaut_minutes = duree_intervention_defaut_minutes;
    if (delai_alerte_retard_minutes !== undefined) updates.delai_alerte_retard_minutes = delai_alerte_retard_minutes;
    if (validation_rapport_obligatoire !== undefined) updates.validation_rapport_obligatoire = validation_rapport_obligatoire;
    if (signature_client_obligatoire !== undefined) updates.signature_client_obligatoire = signature_client_obligatoire;
    if (mode_facturation !== undefined) updates.mode_facturation = mode_facturation;
    if (delai_paiement_jours !== undefined) updates.delai_paiement_jours = delai_paiement_jours;
    if (tva_par_defaut !== undefined) updates.tva_par_defaut = tva_par_defaut;
    if (conditions_generales !== undefined) updates.conditions_generales = conditions_generales;
    if (email_notifications_auto !== undefined) updates.email_notifications_auto = email_notifications_auto;
    if (modeles_email !== undefined) updates.modeles_email = modeles_email;
    if (signature_email !== undefined) updates.signature_email = signature_email;
    if (webhook_actif !== undefined) updates.webhook_actif = webhook_actif;
    if (api_publique_active !== undefined) updates.api_publique_active = api_publique_active;
    if (cle_api_publique !== undefined) updates.cle_api_publique = cle_api_publique;
