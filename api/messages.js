...
export async function markAsRead(req, res) {
  try {
    ...
    if (message.recipient_id !== userId) {
      return res.status(403).json({ error: 'Vous ne pouvez marquer comme lu que vos propres messages reçus' });
    }
...
export async function markConversationAsRead(req, res) {
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({ error: 'Erreur lors du marquage des messages comme lus' });
    }
...
export async function getContextMessages(req, res) {
  try {
    ...
    if (!['ticket', 'mission', 'facture'].includes(type)) {
      return res.status(400).json({ error: 'Type invalide. Utilisez: ticket, mission ou facture' });
    }
  ...
  try {
    ...
    if (diffMinutes > 15) {
      return res.status(403).json({ 
        error: 'Vous ne pouvez supprimer un message que dans les 15 minutes suivant son envoi' 
      });
    }
